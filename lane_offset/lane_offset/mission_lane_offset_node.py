...
- HSV+YCrCb 5색 분류와 최근접 보간 BEV 마스킹
- 횡단보도/정지선 가로 밴드 제거
...
SOURCE_REFERENCE_WIDTH = 640.0

# timed_lane_offset_node의 HSV+YCrCb 5색 분류 설정.
...
class MissionLaneOffsetNode(Node):
    ...
    def __init__(self):
        ...
        # 기존 640px 카메라 기준값을 BEV 하단 좌표로 변환한다.
        self.dashed_reference_x_px_1lane = self.camera_x_to_bev_bottom(
            self.dashed_reference_x_px_1lane
        )
        self.dashed_reference_x_px_2lane = self.camera_x_to_bev_bottom(
    ...
    def set_driving_mode(self, mode, force=False):
        ...
        previous_dashed_start = self.window_start_x.get('dashed')
        ...
        self.window_start_x = {
            'left': None,
            'dashed': (
                float(self.dashed_reference_x_px)
                if force or previous_dashed_start is None
                else float(previous_dashed_start)
            ),
            'right': None,
        }
        self.last_lane_tracks = None
    ...
    def segment_colors(self, frame):
        """원본을 white/light-gray/dark-gray/green 5색 영상으로 분류한다."""
    ...
    def camera_x_to_bev_bottom(self, camera_x):
        normalized_x = float(camera_x) / SOURCE_REFERENCE_WIDTH
        source_x = normalized_x * self.bev_output_width
        center_x = self.bev_output_width / 2.0
        transformed = center_x + (
            source_x - center_x
        ) * self.bev_bottom_width_ratio
        return int(round(transformed))

    # BEV 입력은 이미 고정된 5색이므로 HSV에서 각 고정 색을 정확히 꺼낸다.
    ...
    def make_green_mask(hsv):
        h, s, v = cv2.split(hsv)
        return (
            ((h == 60) & (s == 255) & (v == 255)).astype(np.uint8) * 255
        )

    @staticmethod
    def make_gray_mask(hsv):
        _h, s, v = cv2.split(hsv)
        gray = (s == 0) & ((v == 105) | (v == 211))
